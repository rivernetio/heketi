...
func (vc *VolumeCreateOperation) Exec(executor executors.Executor) error {
	brick_entries, err := bricksFromOp(vc.db, vc.op, vc.vol.Info.Gid, vc.vol.Info.FastMode)
	if err != nil {
...
func (vc *VolumeCreateOperation) Finalize() error {
	return vc.db.Update(func(tx *bolt.Tx) error {
		brick_entries, err := bricksFromOp(wdb.WrapTx(tx), vc.op, vc.vol.Info.Gid, vc.vol.Info.FastMode)
		if err != nil {
			logger.LogError("Failed to get bricks from op: %v", err)
			return err
		}
...
func (ve *VolumeExpandOperation) Exec(executor executors.Executor) error {
	brick_entries, err := bricksFromOp(ve.db, ve.op, ve.vol.Info.Gid, ve.vol.Info.FastMode)
	if err != nil {
		logger.LogError("Failed to get bricks from op: %v", err)
		return err
	}
...
func (ve *VolumeExpandOperation) Finalize() error {
	return ve.db.Update(func(tx *bolt.Tx) error {
		brick_entries, err := bricksFromOp(wdb.WrapTx(tx), ve.op, ve.vol.Info.Gid, ve.vol.Info.FastMode)
		if err != nil {
			logger.LogError("Failed to get bricks from op: %v", err)
			return err
		}
...
func (vdel *VolumeDeleteOperation) Exec(executor executors.Executor) error {
	brick_entries, err := bricksFromOp(vdel.db, vdel.op, vdel.vol.Info.Gid, vdel.vol.Info.FastMode)
	if err != nil {
		logger.LogError("Failed to get bricks from op: %v", err)
		return err
	}
...
func (vdel *VolumeDeleteOperation) Rollback(executor executors.Executor) error {
	...
	return vdel.db.Update(func(tx *bolt.Tx) error {
		txdb := wdb.WrapTx(tx)
		brick_entries, err := bricksFromOp(txdb, vdel.op, vdel.vol.Info.Gid, vdel.vol.Info.FastMode)
		if err != nil {
			logger.LogError("Failed to get bricks from op: %v", err)
			return err
		}
...
func (bvc *BlockVolumeCreateOperation) volAndBricks(db wdb.RODB) (
	...
	if len(volume_entries) == 1 {
		...
		fastMode = volume_entries[0].Info.FastMode
	}
	brick_entries, err = bricksFromOp(db, bvc.op, brickGid, fastMode)
	if err != nil {
		logger.LogError("Failed to get bricks from op: %v", err)
		return
	}
...
func bricksFromOp(db wdb.RODB,
	op *PendingOperationEntry, gid int64, fastMode bool) ([]*BrickEntry, error) {

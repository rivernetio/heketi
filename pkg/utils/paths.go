...
const (
	...
	brickMountPointRootLocal = "/var/lib/heketi/local"
...
func BrickPath(vgId, brickId string, fastMode bool) string {
	return path.Join(
		BrickMountPoint(vgId, brickId, fastMode),
		"brick")
}
...
func BrickMountPoint(vgId, brickId string, fastMode bool) string {
	if !fastMode {
		return path.Join(
			brickMountPointRoot,
			VgIdToName(vgId),
